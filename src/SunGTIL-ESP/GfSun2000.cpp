...
void handleData(ModbusMessage response, uint32_t token) {
  ...
  // output Power
  if (gtilWorkMode != BASIC_MODE)
    response.get(reg2ix(REGISTRY_OUTPUT_POWER), num16);
  else
    response.get(reg2ix(REGISTRY_AVERAGE_POWER), num16);
  data.outputPower = num16 / 10.00;
  ...
  // temperature
  if (gtilWorkMode != BASIC_MODE) {
    response.get(reg2ix(REGISTRY_TEMPERATURE), num16);
    data.temperature = num16 / 10.00;
    // limmiter power
    response.get(reg2ix(REGISTRY_LIMMITER_POWER), num16);
    data.limmiterPower = num16 / 10.00;
  }
  else{
    data.temperature = 30;
    data.limmiterPower = 0;
  }
